...
router.get("/", async (req, res) => {
    try {
        ...
        const limitPerPage = Math.min(100, Math.max(1, parseInt(limit, 10) || 10));
        const offset = (currentPage - 1) * limitPerPage;
        const filterConditions = [];
        if (search) {
            filterConditions.push(or(ilike(patients.name, `%${search}%`), ilike(patients.phone, `%${search}%`)));
        }
        if (status) {
            filterConditions.push(eq(patients.status, status));
        }
